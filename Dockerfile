...
# and `pip install churn` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "churn", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n churn -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "churn", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "churn", "YOUR_CONSOLE_SCRIPT"]
